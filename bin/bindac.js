...
async function main() {
    ...
    if (!isUserDir && !isTmpDir && !isProjectRelative &&
        forbiddenDirs.some((dir) => resolvedOut === dir || resolvedOut.startsWith(dir + path.sep))) {
    ...
    try {
        ...
        if (!zoneConfig)
            throw new Error("No default export found in input file.");
    }
    catch (err) {
